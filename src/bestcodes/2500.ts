...
class ORDERS {
    ...
    public static readonly runWind = new ParentOrder('lancer le sort de vent pour repousser une orde en defense', this.i++, [HEROTYPE.def,HEROTYPE.explore], ORDERTYPE.spell)
...
function defZonesContent() : Position[] {
    let result : Position[] = []
    if (baseX == 0) {
        result.push(new Position(3800, 2200))
        result.push(new Position(2800, 4200))
        result.push(new Position(5000, 1000))

    } else {
        result.push(new Position(maxX-3800, maxY-2200))
        result.push(new Position(12000, 7500))
        result.push(new Position(15000, 4000))
    }
    return result
}
...
function attZonesContent() : Position[] {
    ...
    if (baseX == 0) {
        ...
        result.push(new Position(3600, 2200))
        result.push(new Position(2200, 4500))
        result.push(new Position(5000, 1200))
    }
    return result
}
...
function orderFactory(entities: Entity[]) : GlobalState[] {
    ...
    getHeroes().forEach(hero => {
        ...
        let monsterOpposedBaseFocus = getMonsterOpposedBaseFocus(monsters)
        ...
        let focusOrders: Order[] = [
            ...
            new Order(ORDERS.focusMonsterProximityForShield, hero,
                monsterOpposedBaseFocus.length > 0 &&
                hero.targetId == null &&
        ...
        let orders: Order[] = [
            new Order(ORDERS.runWind, hero,
                ...
                (monsterInWindZone.length >= 2 || vectorSizeEntityBase(monsters[0])<baseSize/2)),
            ...
            new Order(ORDERS.moveToOpposedBase, hero, !isInOpposedBase(hero) && turn > turnChangeStrat && myMana > 100),
...
function executeOrders(globalState:GlobalState[]){
    ...
    globalState.forEach(globalState => {
        ...
        if(globalState.hero.targetId != null){
            console.error("################ FOCUS ID " +  globalState.hero.targetId.id+"x "+globalState.hero.targetId.x +" y "+  +"#####################################")
        }else {
...
function heroRoleAttribution(entities: Entity[], heros: Map<number, MyHero>) {
    ...
    sortProximityBase(Array.from(heros.values())).forEach(hero => {
            ...
            if(turn < farmTurn) {
                hero = new ExploreGard(hero.id, hero.x, hero.y, hero.shieldLife, hero.isControlled, hero.health, hero.vx, hero.vy, hero.nearBase, hero.threatFor, hero.targetId)

            }else if (indexDef < getDefByDefaultBySumMonsterFocus(monsterBaseFocus.length)) {
                hero = new DefGard(hero.id, hero.x, hero.y, hero.shieldLife, hero.isControlled, hero.health, hero.vx, hero.vy, hero.nearBase, hero.threatFor, hero.targetId);
                indexDef++;
            } else if (indexAtt < attByDefault && turn >= turnChangeStrat && myMana > 30) {
                hero = new AttGard(hero.id, hero.x, hero.y, hero.shieldLife, hero.isControlled, hero.health, hero.vx, hero.vy, hero.nearBase, hero.threatFor, hero.targetId)
                indexAtt++;
                idAtt = hero.id
            } else {
                hero = new ExploreGard(hero.id, hero.x, hero.y, hero.shieldLife, hero.isControlled, hero.health, hero.vx, hero.vy, hero.nearBase, hero.threatFor, hero.targetId)
...
function getTypeEntityByInputs() {
    ...
    switch (type) {
        ...
        case HEROES :
            return new MyHero(id, x, y, new Position(-1, -1), new Position(-1, -1),new Position(-1, -1), shieldLife, isControlled, health, vx, vy, nearBase, threatFor)
        case HEROESENNEMY :
